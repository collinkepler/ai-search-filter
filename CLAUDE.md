...
| **1 — Image** | image-scanner.js, IntersectionObserver on `<img>`/`<video>`. **Preemptive blur**: every observable image is blurred immediately on `observe()`; classification runs on intersect; on `nsfw=true` the image is hidden (`visibility: hidden`), on `nsfw=false` the blur is cleared. On `failClosed=true` (image un-fetchable) the `imageUnverifiedAction` setting decides reveal/blur/hide. Categories include `suggestive` alongside `porn` — both return `nsfw=true`. Strictness is set by `imageStrictness`. | Service worker `classifyImage` (Claude vision) | `aisf-img-cache`, 30 days, keyed by `imageStrictness` tier + image URL hash. Invalidated by `IMG_CACHE_VERSION` bump (see Caching). |
...
**Feed-container exemption.** `extractContent` returns `null` for YouTube non-watch container pages (homepage, channels, shorts, `/feed/*`) so Layer 0 is skipped entirely — the page reveals immediately and Layer 3 filters individual video cards as they scroll into view. This is deliberate: an only-allow rule like "real-estate videos" would otherwise block the bare YouTube homepage (no specific video to classify against). `/watch?v=...` URLs flow through Layer 0 as `youtube_video`. **Search results (`/results?search_query=...`) are *not* exempted** — they go through the generic search-query detector (`detectSearchQuery` in [content-script.js](content-script.js)) as `{type: 'search'}`, identical to Google/Bing/DuckDuckGo. Trade-off: a user who wants to block YouTube wholesale still can't do it via a Layer 0 block rule on `youtube.com/`; they have to rely on Layer 3, an external blocker, or a block rule matching watch-page or search-query content.

**Generic search-query detection.** `detectSearchQuery` ([content-script.js](content-script.js)) classifies any URL containing a recognizable search param as `{type: 'search', engine: hostname, query}` — covering YouTube, Reddit, Twitter/X, GitHub, Stack Overflow, Amazon, eBay, Wikipedia, TikTok, Pinterest, Twitch, Pornhub, etc., plus the legacy Google/Bing/DuckDuckGo cases. Two-tier param list: STRONG params (`q`, `query`, `search_query`, `k`, `_nkw`, etc.) fire on any path; WEAK params (`s`, `term`, `search`, etc.) only fire when the path also matches `SEARCH_PATH_RE` (`/search`, `/results`, `/find`, `/sch`, etc.) to avoid mistaking sort/pagination filters for searches. Verdicts cache by `search:<lowercased query>`, so the same query gets one classification across all sites.
...
Two stores in `chrome.storage.local`: `aisf-cache` (Layer 0) and `aisf-img-cache` (Layer 1). Both use the generic `getCached`/`setCached` helpers with LRU-style eviction by oldest timestamp. Layer 0 cache keys incorporate a `hashStr` of the rules JSON so that editing rules implicitly invalidates verdicts; the options page also explicitly removes `aisf-cache` on Save ([options.js](options.js)). Layer 1 cache keys incorporate the `imageStrictness` tier (`img:<tier>:<urlhash>`) so switching strictness re-classifies. Image classification errors are never cached (the `classifyImage` catch path skips `setCached`), so a transient fetch failure retries on the next visit. When changing what data feeds a classification, update the cache key accordingly or stale verdicts will persist.

**Cache versioning for prompt changes.** Layer 0 uses `CACHE_VERSION`, Layer 1 uses `IMG_CACHE_VERSION`, and the per-host image-skip cache uses `HOST_SKIP_CACHE_VERSION` — all constants at the top of [service-worker.js](service-worker.js), each with a startup IIFE that compares against the stored version key and wipes the matching store on mismatch. **Bump the relevant constant any time its system prompt changes semantically** (e.g. adding a category, broadening what counts as NSFW, adding a personalContext block) — otherwise users keep getting stale verdicts from the old prompt.
...
- `enableSiteFilters` / `enableImageScanner` / `enablePostScanner` — Layer toggles (Layer 1 and Layer 2 default ON, Layer 3 default OFF)
- `imageAction` (`blur`/`hide`), `imageMinSize`, `postAction` (`hide`/`dim`)
- `imageStrictness` — Layer 1 image-scanner tier: `'standard'` (context-aware, allows non-sexualized beach/family swimwear), `'strict'` (default — every bikini/swimsuit/lingerie/underwear photo is NSFW regardless of context), `'maximum'` (also flags cleavage, shirtless people, bare midriffs, revealing activewear). Read in `classifyImage` and prepended to the `callClaudeForImage` prompt via `IMAGE_STRICTNESS_DIRECTIVES` (mirrors how Layer 0 prepends `buildStrictnessDirective`). The image cache key folds in the strictness tier (`img:<tier>:<urlhash>`) so switching it re-classifies.
- `imageUnverifiedAction` — what Layer 1 does when Claude cannot fetch/decode an image (hotlink-protected CDNs, dead URLs): `'reveal'` (default), `'blur'` (permanent blur), `'hide'`. `classifyImage` flags these as `failClosed: true` only on `API 400` errors (the image's fault); 401/403/429/5xx/network errors stay fail-open. The content script (`AISFImageScanner`) maps the flag to the action via `applyUnverified`. A one-time migration moves legacy unset/`hide` values to `reveal` so protected poster grids do not disappear without an actual NSFW verdict.
- `personalContext` — optional free-text "About you" identity blurb. Always used by the Sonnet appeal review (`callClaudeForAppeal` in [service-worker.js](service-worker.js)) as ground-truth identity so appeals claiming a profession/identity the user has not stated default to UPHOLD; there it's injected into both system prompt and user message. **Also used by Layer 0 (`callClaudeForContext`) and Layer 3 (`callClaudeForPosts`) when the toggle `personalContextOnHotPaths` is on** — injected as an "ABOUT THE USER" block at the top of the system prompt only (not the user message, which carries the content under classification). The shared injection point is `buildPersonalContextBlock` in [service-worker.js](service-worker.js), which hard-caps the blurb at `PERSONAL_CONTEXT_MAX_CHARS` (2000) to bound Layer 3 token cost. The Layer 0 cache key incorporates `hashStr(personalContext)` so toggling the setting or editing the blurb invalidates verdicts. Omitted entirely from prompts when empty.
- `personalContextOnHotPaths` — boolean, default `true`. Controls whether `personalContext` is fed to Layer 0 and Layer 3. Does not affect appeals (those always use it).
